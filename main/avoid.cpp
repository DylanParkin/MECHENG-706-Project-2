...
void Avoid() {
  ...
  if (IR_front_left < IR_front_right) {
    SerialCom->print("Determined strafe right by front sensors FL: ");
    SerialCom->print(IR_front_left);
    SerialCom->print("   FR: ");
    SerialCom->println(IR_front_right);
    strafe_right = true;
  } else if (IR_right < 50.0f || IR_left < 50.0f) {
    SerialCom->print("Determined strafe direction via side sensors. (within range to hit something)   L: ");
  ...
  while (true) {
    ...
    // reverse strafe direction if close to something
    ...
    SerialCom->print("front clearance IR: ");
    SerialCom->println(front_clearance_IR);

    if (front_clearance_IR >= high_clearance_threshold) {
      delay(200);
      stop();
      break;
    }
    ...
    left_front_motor.writeMicroseconds(1500 - dir * strafe_speed - (int)correction);
    left_rear_motor.writeMicroseconds(1500 + dir * strafe_speed - (int)correction);
    right_rear_motor.writeMicroseconds(1500 + dir * strafe_speed - (int)correction);
    right_front_motor.writeMicroseconds(1500 - dir * strafe_speed - (int)correction);
  ...
  while (true) {
    ...
    if (side_clearance_IR < low_clearance_threshold) {
      side_was_low = true;
    } else if (side_was_low && side_clearance_IR >= high_clearance_threshold) {
      stop();
      break;
    }

    if (object_detected() == OBSTACLE) {
      stop();
      Avoid();
      return;
    } else {
