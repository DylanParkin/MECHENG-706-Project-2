...
This file is almost identical to the provided file RobotBaseCodes2026.ino, with some small modifications.

Below is a running list of all the modifications made to the original file.

1. speed_val variable changed from 100 to 350

2. strafe_speed variable added

3. setup and loop moved to main.ino, and extended

4. several changes made to initialising() function. Now returns the state "HOMING"

5. modified HC_SR04_range() function to return a float

6. Gyro function changed to return a float

7. State variables moved into states.h, and added homing and tilling states

8. running function was combined with initialising function, now just initialising()
...
const int TRIG_PIN = 5;  // 48
const int ECHO_PIN = 2;  // 49
...
// SH2_GYROSCOPE_CALIBRATED, 10000
// SH2_GYROSCOPE_UNCALIBRATED, 10000
// SH2_GAME_ROTATION_VECTOR, 5000
...
#ifndef NO_READ_GYRO
    gyroReading = GetHeading();  // slightly different function to what's in base
                                 // code. Returns a float
#endif

#ifndef NO_HC - SR04
    ultraReading = HC_SR04_range();  // modified. returns cm in float.
#endif
